...
class BlacklistManager(Manager):
    ...
    @staticmethod
    def retrieve_blacklist_by_id(user_id) -> List[int]:
        Manager.check_none(user_id=user_id)
        blacklist = Blacklist.query.filter(or_(Blacklist.blocking_user_id == user_id,Blacklist.blocked_user_id == user_id,)).all()
    ...
    def retrieve_blacklist_element(blocking, blocked) -> Blacklist:
        ...
        blacklist = Blacklist.query.filter(Blacklist.blocking_user_id == blocking).filter(Blacklist.blocked_user_id==blocked).first()
